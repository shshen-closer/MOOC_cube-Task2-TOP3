...
def Resnet(x, drop_rate, span, is_training):
    ...
    with tf.name_scope("block1"):
        for i in range(6):
          ...
          #  x = tf.compat.v1.layers.max_pooling1d(x, pool_size=2,strides=2,padding="SAME")
...
def multi_span(x, drop_rate, is_training):
   ...
   # with tf.name_scope("flatten"):
      #  x = tf.nn.relu( tf.compat.v1.layers.batch_normalization(x, momentum=0.9, epsilon=1e-5, training=is_training))
  #  x = tf.nn.relu(tf.compat.v1.layers.dense(x, 256))
       # x = tf.reduce_mean(x, axis = 1)
